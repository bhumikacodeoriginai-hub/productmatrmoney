...
function renderStep(step: number, uploads: UploadItem[], handleUploads: (event: ChangeEvent<HTMLInputElement>) => void, setUploads: (items: UploadItem[]) => void) {
  ...
  if (step === 7) return <><label className="form-field"><span>Profile category</span><select className="form-input"><option>General matrimony</option><option>Physical / Locomotor</option><option>Hearing & Speech Impaired</option><option>Vitiligo / Skin condition specific</option></select></label><Field label="Anything you’d like people to know?" placeholder="Optional — share only what feels right" textarea /><p className="muted" style={{ fontSize: 11, display: "flex", gap: 6, alignItems: "center" }}><LockKeyhole size={14} /> This is private information. You choose who can see it.</p></>;
