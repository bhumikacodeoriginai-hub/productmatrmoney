...
import { Check, Heart, Pause, Play, ShieldCheck, Star, Video } from "lucide-react";
...
export function MediaPreview({ media, className = "", showControls = true }: { media: ProfileMedia[]; className?: string; showControls?: boolean }) {
  const item = media[0];
  const videoRef = useRef<HTMLVideoElement>(null);
  const [playing, setPlaying] = useState(item?.kind === "video");
  ...
  return <div className={`media-preview ${className}`}>
    {item.kind === "video" ? <video ref={videoRef} className="media-preview-element" src={item.src} poster={item.poster} muted loop playsInline autoPlay preload="metadata" aria-label={item.alt} /> : <img className="media-preview-element" src={item.src} alt={item.alt} loading="lazy" />}
...
export function ProfileCard({ profile, compact = false }: { profile: Profile; compact?: boolean }) {
  ...
  return <article className="profile-card">
    <div className="profile-image-wrap"><MediaPreview media={media} /><VerifiedBadge /><button className="icon-btn profile-heart" aria-label={`${saved ? "Remove" : "Add"} ${profile.name} ${saved ? "from " : "to "} shortlist`} onClick={() => setSaved(!saved)}><Star size={17} fill={saved ? "currentColor" : "none"} /></button></div>
